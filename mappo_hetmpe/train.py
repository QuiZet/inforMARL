...
def train(cfg: DictConfig):
    ...
    env = make_env()
    policies = initialize_agents(env, cfg.train.share_policy)
    ...
    if cfg.train.load_model:
        for agent in agents:
            model_path = os.path.join(cfg.train.load_model, f"{agent}_policy_model.pth")
            model_type = 'adversary' if 'adversary' in agent else 'agent'
            policies[agent].load_model(model_path, model_type=model_type)

    output_dir = cfg.train.output_dir or f"models/run_{datetime.now().strftime('%Y%m%d_%H%M%S')}"
