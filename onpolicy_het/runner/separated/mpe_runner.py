...
from onpolicy_het.utils.util import update_linear_schedule
from onpolicy_het.runner.separated.base_runner import Runner
import imageio
...
class MPERunner(Runner):
    ...
    def warmup(self):
        # reset env
        obs = self.envs.reset()
        for i in range(len(obs)): # thread
            for j in range(len(obs[i])): # agent
                print(f'obs[{i}/{j}]:{obs[i][j].shape}')
        ...
        for agent_id in range(self.num_agents):
            ...
            b = np.array(b)
            print(f'warmup self.buffer.obs:{self.buffer[agent_id].obs.shape} obs[0]:{self.buffer[agent_id].obs[0].shape} type:{type(self.buffer[agent_id].obs.shape)} {type(self.buffer[agent_id].obs[0].shape)}')
            print(f'warmup b:{b.shape} type:{type(b)}')
            self.buffer[agent_id].obs[0] = b.copy()
