...
from onpolicy_het.config import get_config
from multiagent_het.MPE_env import MPEEnv, GraphMPEEnv
from onpolicy_het.envs.env_wrappers import (
    SubprocVecEnv,
    DummyVecEnv,
    GraphSubprocVecEnv,
    GraphDummyVecEnv,
...
def make_train_env(all_args: argparse.Namespace):
    print(f'make_train_env:{all_args}')
...
def main(args):
    ...
    if all_args.share_policy:
        if all_args.env_name == "GraphMPE":
            from onpolicy_het.runner.shared.graph_mpe_runner import GMPERunner as Runner
        else:
            from onpolicy_het.runner.shared.mpe_runner import MPERunner as Runner
    else:
        if all_args.env_name == "GraphMPE":
            raise NotImplementedError
        from onpolicy_het.runner.separated.mpe_runner import MPERunner as Runner
