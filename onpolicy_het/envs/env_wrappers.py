...
from onpolicy_het.utils.util import tile_images
...
class SubprocVecEnv(ShareVecEnv):
    ...
    def reset_ng(self):
        ...
        for i in range(len(obs)):
            ...
            #    print(f'lo{i} {k}:{obs[i][k].shape}')
            concatenated_array = np.concatenate(obs[i], axis=0)
            #print(f'concatenated_array:{concatenated_array.shape}')
            #concatenated_array = np.tile(concatenated_array, len(obs[i]))
            #print(f'concatenated_array:{concatenated_array.shape}')
            concatenated_arrays.append(concatenated_array)
        return concatenated_arrays
        #print(f'concatenated_arrays:{concatenated_arrays}')
        obs = np.concatenate(concatenated_arrays, axis=0)
        return obs
        #return np.stack(obs)
...
class DummyVecEnv(ShareVecEnv):
    ...
    def step_wait(self):
        ...
        rews, dones, infos = map(np.array, [rews, dones, infos])
