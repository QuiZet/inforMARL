...
from multiagent_temp.core import World, Agent
from multiagent_temp.multi_discrete import MultiDiscrete
...
class MultiAgentBaseEnv(gym.Env):
    ...
    def __init__(
        ...
        for agent in self.agents:
            obs_dim = len(observation_callback(agent=agent, world=self.world))
            obs_dim_list.append(obs_dim)
        share_obs_dim = max(obs_dim_list) * len(self.agents)
        self.share_observation_space = [
            spaces.Box(
    ...
    def render(self, mode: str = "human", close: bool = False) -> List:
        ...
        for i in range(len(self.viewers)):
            ...
            if self.viewers[i] is None:
                ...
                from multiagent_temp import rendering
