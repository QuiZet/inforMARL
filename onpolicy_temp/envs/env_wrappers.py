...
from onpolicy_temp.utils.util import tile_images
    ...
    for agent_obs in obs:
        ...
        for obs_dim in agent_obs:
            if len(obs_dim) < max_obs_dim:
                pad_start = (max_obs_dim - len(obs_dim)) // 2
                pad_end = max_obs_dim - len(obs_dim) - pad_start
                padded_obs_dim = np.pad(obs_dim, (pad_start, pad_end), mode='constant')
...
class SubprocVecEnv(ShareVecEnv):
    ...
    def step_wait(self):
        results = [remote.recv() for remote in self.remotes]
        self.waiting = False
        obs_raw, rews_raw, dones, infos = zip(*results)
        obs = np.stack(pad_obs(obs_raw))
        rews = np.stack(rews_raw)
        rews = rews.reshape(rews.shape[0], rews.shape[1], 1)
        return obs, rews, np.stack(dones), infos

    def reset(self):
        for remote in self.remotes:
            remote.send(("reset", None))
        obs = [remote.recv() for remote in self.remotes] # len(obs) = 32 for simple_tag
        #cleanup
        #for i in range(len(obs)):
        #    for j in range(len(obs[i])):
        #        print(f'obs size {i} {j}: {obs[i][j].shape}')
        padded_obs = pad_obs(obs)
        #cleanup
        #for i in range(len(padded_obs)):
        #    for j in range(len(obs[j])):
        #        print(f'padded_obs size {i} {j}: {padded_obs[i][j].shape}')
        return np.stack(padded_obs)
