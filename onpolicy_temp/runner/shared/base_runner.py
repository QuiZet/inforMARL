...
from onpolicy_temp.utils.graph_buffer import GraphReplayBuffer
...
class Runner(object):
    ...
    def __init__(self, config: Dict):
        ...
        if self.all_args.env_name == "GraphMPE":
            from onpolicy_temp.algorithms.graph_mappo import GR_MAPPO as TrainAlgo
            from onpolicy_temp.algorithms.graph_MAPPOPolicy import GR_MAPPOPolicy as Policy
        else:
        ...
        if self.use_centralized_V: #default True
            ...
            #print(f'used self observation space: {self.envs.share_observation_space[0]}')
        else:
        ...
        if self.all_args.env_name == "GraphMPE":
            ...
            self.policy = Policy(
                self.all_args,
                self.envs.observation_space[0], # NOTE: keep as is when using pad/tranquil
                share_observation_space,
                self.envs.action_space[0], # NOTE: change here
                device=self.device,
            )
        ...
        if self.all_args.env_name == "GraphMPE":
            ...
            self.buffer = SharedReplayBuffer(
                self.all_args,
                self.num_agents,
                self.envs.observation_space[0], # NOTE: change here
                share_observation_space,
                self.envs.action_space[0], 
            )
